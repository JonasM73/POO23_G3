...
{
	return "SELECT [c].[Numero_Client],[c].[nom_client],[c].[prenom_client],[c].[DNN_CLient], [c].[Date_Pachat_Client]," +
...
System::String^ NS_Comp_MappageClient::CLmapTBClient::Delete(void) {
	return  "DELETE FROM [Paiement] " +
		"WHERE [id_commande] IN (SELECT [id_commande] FROM [Commande] WHERE [id_client] IN (SELECT [id_client] FROM [Client] WHERE [nom_client] = '" + this->nom_client + "' AND [prenom_client]='" + this->prenom_client + "' AND [DNN_client] = '" + this->DNN_client + "' AND [Numero_Client] = '" + this->numero_client + "')); " +
		"DELETE FROM [Commande_Article] " +
		"WHERE [id_commande] IN (SELECT [id_commande] FROM [Commande] WHERE [id_client] IN (SELECT [id_client] FROM [Client] WHERE [nom_client] = '" + this->nom_client + "' AND [prenom_client]='" + this->prenom_client + "' AND [DNN_client] = '" + this->DNN_client + "' AND [Numero_Client] = '" + this->numero_client + "')); " +
		"DELETE FROM [Commande] " +
		"WHERE [id_client] IN (SELECT [id_client] FROM [Client] WHERE [nom_client] = '" + this->nom_client + "' AND [prenom_client]='" + this->prenom_client + "' AND [DNN_client] = '" + this->DNN_client + "' AND [Numero_Client] = '" + this->numero_client + "'); " +
		"DELETE FROM [Client] " +
		"WHERE [nom_client] = '" + this->nom_client + "' AND [prenom_client]='" + this->prenom_client + "' AND [DNN_client] = '" + this->DNN_client + "' AND [Numero_Client] = '" + this->numero_client + "'; " +
		"DELETE a FROM [Adresse] a " +
		"WHERE EXISTS (SELECT 1 FROM [Client] c WHERE ([c].[adresse_F]=[a].[id_adresse] OR [c].[adresse_L] = [a].[id_adresse]) " +
		"AND [c].[nom_client] = '" + this->nom_client + "' AND [c].[prenom_client] = '" + this->prenom_client + "' AND [c].[DNN_client] = '" + this->DNN_client + "' AND [c].[Numero_Client] = '" + this->numero_client + "');";
}
